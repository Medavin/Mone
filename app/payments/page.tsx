...
export default async function PaymentsPage({
  ...
  const [clinicRes, peopleRes] = await Promise.all([
  ...
  const { rows } = await fetchAllRows<Record<string, unknown>>((lo, hi) =>
    supabase.from("reported_payments").select("*")
      .gte("reported_on", from).lte("reported_on", to)
      .order("reported_on", { ascending: false }).range(lo, hi)
  );
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
        {!profile ? (
          <p className="text-sm text-muted">Sign in to see reported payments.</p>
        ) : (
          <PaymentsClient
